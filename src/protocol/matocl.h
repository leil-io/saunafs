...
// healthFromScan says whether this server measures the two states above rather than keeping
// them current as chunks change; only then does freshness describe anything, and only then does
// an unset generation mean nothing has been measured yet. serverTime is the answering server's
// clock, so a client can age the measurement without comparing it against its own.
