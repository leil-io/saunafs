...
/// Re-emits gChunkChangedSignal for every live chunk and returns the number
/// emitted. Used when bounded promotion tracking overflows and the complete
/// chunk section must be reconstructed from the promoted master's memory.
uint64_t chunk_emit_all_changed();
