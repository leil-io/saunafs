...
public:
	...
	/// How the writer drains its queue. A scoped enum rather than a bool so a stray positional
	/// argument cannot silently select the wrong mode.
	enum class WriterMode : uint8_t {
		kSynchronous,  ///< The caller drives flush() (e.g. the migration bootstrap).
		kAsync,        ///< A background worker thread drains and commits the queue.
	};
	...
	/// @param mode              kAsync starts a background worker thread that drains and commits
	///        the queue off the caller's thread, so enqueue() never blocks on FDB commit latency.
	///        kSynchronous (the default, e.g. the migration bootstrap) leaves the caller to drive
	///        flush().
	...
	explicit MetadataWriterFDB(kv::IKVEngine *kvEngine,
	                           MetadataCheckpointManager *checkpointManager = nullptr,
	                           WriterMode mode = WriterMode::kSynchronous,
