                                     ...
                                     unsigned int mode)
{
    ...
    lzfs_acl = liz_create_acl_from_mode(mode);
    if (!lzfs_acl) {
        return NULL;
    }
        ...
        if (IS_FSAL_ACE_SPECIAL_ID(*fsal_ace)) {
            ...
            default:
                LogFullDebug(COMPONENT_FSAL,
                             "ALLI: Invalid FSAL ACE special id type (%d)",
...
fsal_acl_t *lzfs_int_convert_lzfs_acl(const liz_acl_t *lzfs_acl)
{
    ...
    for (size_t i = 0; i < acl_data.naces; ++i) {
        ...
        if (IS_FSAL_ACE_SPECIAL_ID(*fsal_ace)) {
            ...
            default:
                ...
                LogWarn(COMPONENT_FSAL,
                        "ALLI: Invalid LizardFS ACE special id type (%u)",
                              ...
                              fsal_acl_t **fsal_acl)
{
    ...
    *fsal_acl = lzfs_int_convert_lzfs_acl(acl);
    ...
    if (*fsal_acl == NULL) {
        LogFullDebug(COMPONENT_FSAL,
                     "ALLI: Failed to convert lzfs acl to nfs4 acl, export=%"
                     PRIu16 " inode=%" PRIu32,
                     export->export.export_id, inode);
                              ...
                              unsigned int mode)
{
    ...
    liz_acl_t *lzfs_acl = lzfs_int_convert_fsal_acl(fsal_acl, mode);
