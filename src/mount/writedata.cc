...
struct ChunkData {
	...
	inodedata *parent; // contains inode and other useful data
	ChunkData *next;
	...
	void updateParentStatus(int8_t status) { parent->status = status; }
	...
	bool requiresFlushing() const {
		return parent->requiresFlushing();
	}

	void clear() {
		parent->totalCachedBlocks -= dataChain.size();
		dataChain.clear();
	}
...
void write_job_delayed_end(ChunkData* chunkData, int status, int seconds, Glock &lock) {
	...
	if (status != SAUNAFS_STATUS_OK) {
		safs_pretty_syslog(LOG_WARNING, "error writing file number %" PRIu32 ": %s", chunkData->parent->inode, saunafs_error_string(status));
...
public:
	InodeChunkWriter() : chunkData_(nullptr), chunkIndex_(0) {}
...
void InodeChunkWriter::processJob(ChunkData* chunkData) {
	...
	try {
		try {
			locator->locateAndLockChunk(chunkData_->parent->inode, chunkIndex_);
			Glock lock(gMutex);
			chunkData_->parent->maxfleng =
			    std::max(chunkData_->parent->maxfleng, locator->fileLength());
			lock.unlock();
			...
			locator->unlockChunk();
			read_inode_ops(chunkData_->parent->inode);
			...
			safs_pretty_syslog(LOG_WARNING, "write file error, inode: %" PRIu32 ", index: %" PRIu32 " - %s",
					chunkData_->parent->inode, chunkIndex_, errorString.c_str());
...
void InodeChunkWriter::processDataChain(ChunkWriter& writer) {
	...
	while (true) {
		...
		writer.setChunkSizeInBlocks(
		    std::min(chunkData_->parent->maxfleng - chunkIndex_ * SFSCHUNKSIZE,
		             (uint64_t)SFSCHUNKSIZE));
		if (writer.startNewOperations(can_expect_next_block) > 0) {
			Glock lock(gMutex);
			chunkData_->parent->lastWriteToChunkservers.reset();
		}
...
void* write_worker(void*) {
	...
	for (;;) {
		...
		inodeDataWriter.processJob((ChunkData*) data);
	}
	return NULL;
}
...
int write_block(ChunkData* chunkData, uint16_t pos, uint32_t from, uint32_t to, const uint8_t *data) {
	Glock lock(gMutex);
	chunkData->parent->lastWriteToDataChain.reset();
	...
	write_cb_wait_for_block(chunkData->parent, lock);
	write_cb_acquire_blocks(1, lock);
	chunkData->pushToChain(WriteCacheBlock(chunkData->chunkIndex, pos, WriteCacheBlock::kWritableBlock));
