...
TEST_F(ChunkTrashManagerImplTest, CleanEmptyFolders) {
	...
	ASSERT_FALSE(fs::exists(emptyValidDirPath)); // Ensure empty valid dir is removed
	ASSERT_TRUE(fs::exists(emptyInvalidDirPath)); // Ensure empty invalid dir is not removed
}
