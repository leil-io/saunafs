...
protected:
	fs::path testDir;
	ChunkTrashManager::ChunkTrashManagerImpl chunkTrashManagerImpl;
...
TEST_F(ChunkTrashManagerImplTest, CleanEmptyFolders) {
	fs::path emptyDirPath = testDir / "empty_dir";
	fs::create_directory(emptyDirPath);

	chunkTrashManagerImpl.cleanEmptyFolders(testDir.string());

	ASSERT_FALSE(fs::exists(emptyDirPath)); // Ensure empty folder is cleaned up
}
