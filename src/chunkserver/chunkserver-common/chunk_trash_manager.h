...
public:
	 ...
	 */
	void init(const std::string &diskPath);
	...
	// Forward declaration of the implementation details
	class ChunkTrashManagerImpl;

private:
	// Constructor is private to enforce singleton behavior
	ChunkTrashManager(); ///< Default constructor.

	std::unique_ptr<ChunkTrashManagerImpl> pImpl; ///< Pointer to the implementation details.
