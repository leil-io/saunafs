...
void ChunkTrashIndex::remove(const time_t &deletionTime,
	...
	for (const auto &diskEntry: trashIndex) {
		if (filePath.starts_with(diskEntry.first + "/" +
		                         ChunkTrashManager::kTrashDirname)) {
			remove(deletionTime, filePath, diskEntry.first);
			return;
		}
	}
}
...
size_t ChunkTrashIndex::getExpiredFiles(const std::filesystem::path &diskPath,
                                        const time_t &timeLimit,
                                        ChunkTrashIndex::TrashIndexDiskEntries &expiredFiles,
                                        size_t bulkSize) {
//	std::scoped_lock<std::mutex> const lock(trashIndexMutex);
...
//	std::scoped_lock<std::mutex> const lock(trashIndexMutex);
	...
	size_t count = 0;
	for (const auto &diskEntry: trashIndex) {
		count += getExpiredFiles(diskEntry.first, timeLimit, expiredFiles,
		                         bulkSize);
		if (bulkSize != 0 && count >= bulkSize) {
			break;
		}
