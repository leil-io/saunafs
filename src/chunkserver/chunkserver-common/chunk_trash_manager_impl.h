...
class ChunkTrashManager::ChunkTrashManagerImpl {
...
public:
	...
	static constexpr const size_t kGarbageCollectorBulkSize = 1000;
	 ...
	 */
	int init(const std::filesystem::path &diskPath);
	 ...
	 */
	void makeSpace(const size_t spaceAvailabilityThreshold,
	 ...
	 */
	time_t getTimeFromString(const std::string &timeString);
...
private:
	...
	static const int kAvailableThresholdGB;

	/// Time limit (in seconds) for files to be considered eligible for deletion.
	static const int kTrashTimeLimitSeconds;

	/// Number of files processed in each bulk operation during garbage collection.
	static const int kTrashGarbageCollectorBulkSize;

	/// Number of files to remove in each step to free up space when required.
	static const int kGarbageCollectorSpaceRecoveryStep;
};
