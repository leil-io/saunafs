...
ganesha_report="$(fio --name=sequential_write_ganesha --directory=${TEMP_DIR}/mnt/ganesha --size=500M --rw=write --numjobs=1 --ioengine=libaio --group_reporting --bs=4M --direct=1 --iodepth=4)"
...
linuxClient_report="$(fio --name=name=random_mix_rw_linux --directory=${info[mount0]}/linux --size=500M --rw=write --numjobs=1 --ioengine=libaio --group_reporting --bs=4M --direct=1 --iodepth=4)"
