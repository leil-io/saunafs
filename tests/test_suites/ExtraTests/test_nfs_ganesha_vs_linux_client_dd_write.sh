...
ganesha_report="$(dd if=/dev/random bs=500M count=1 of=${TEMP_DIR}/mnt/ganesha/file.dd status=progress 2>&1)"

# print Ganesha write statistics
echo "${ganesha_report}"

ganesha_write_speed="$(echo "${ganesha_report}" | tail -1 | rev | cut -d' ' -f1,2 | rev)"

echo ""
echo "Running dd write on top of Linux Client:"

# Run dd write on top of Linux Client
linuxClient_report="$(dd if=/dev/random bs=500M count=1 of=${info[mount0]}/linux/file.dd status=progress 2>&1)"

# print Linux Client write statistics
echo "${linuxClient_report}"

linuxClient_write_speed="$(echo "${linuxClient_report}" | tail -1 | rev | cut -d' ' -f1,2 | rev)"
