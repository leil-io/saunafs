...
ganesha_report="$(dd if=${TEMP_DIR}/mnt/ganesha/file.dd bs=500M count=1 of=/dev/null status=progress 2>&1)"

# print Ganesha read statistics
echo "${ganesha_report}"

ganesha_read_speed="$(echo "${ganesha_report}" | tail -1 | rev | cut -d' ' -f1,2 | rev)"

echo ""
echo "Running dd read on top of Linux Client:"

# Run dd read on top of Linux Client
linuxClient_report="$(dd if=${info[mount0]}/file.dd bs=500M count=1 of=/dev/null status=progress 2>&1)"

# print Linux Client write statistics
echo "${linuxClient_report}"

linuxClient_read_speed="$(echo "${linuxClient_report}" | tail -1 | rev | cut -d' ' -f1,2 | rev)"
