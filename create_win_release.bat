...
:: Create Makefile
cmake -G "MinGW Makefiles" .. -DCMAKE_BUILD_TYPE=Release -DENABLE_TESTS=NO -DENABLE_DOCS=YES -DENABLE_URAFT=NO -DENABLE_POLONAISE=NO -DENABLE_CCACHE=NO -DENABLE_NFS_ACL_SUPPORT=NO -DENABLE_WINFSP_WINCLIENT=YES -DWINCLIENT_PACKAGE_VERSION=%WINCLIENT_VERSION% -DPACKAGE_VERSION=%SAUNAFS_VERSION% -DWINFSP_VERSION=%WINFSP_VERSION% -Dyaml-cpp_DIR=C:\Users\David\Downloads\yaml-cpp-0.8.0\yaml-cpp-0.8.0\lib\cmake\yaml-cpp
